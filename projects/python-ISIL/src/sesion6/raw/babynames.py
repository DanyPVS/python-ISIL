...
#https://github.com/main1S/Python-isil/tree/master/projects/python-isil/src
...
def extract_names(filename):
    ...
    if not year:
        ...
        for rank in tuples:
            (rank, nino, nina) = rank_tuple
            if nino not in nombres_to_rank:
                nombres_to_rank[nino] = rank
            if nina not in nombres_to_rank:
                nombres_to_rank[nina] = rank
        sorted_nombres = sorted(nombres_to_rank.keys())
        for nombre in sorted_nombres:
            nombres.append(nombre + " " + nombres_to_rank[name])
